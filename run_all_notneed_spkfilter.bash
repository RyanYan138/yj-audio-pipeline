...
if [[ "$DO_DNSMOS" -eq 1 ]]; then
  ...
  # 如果合并后的文件已经存在，整个 Step2 跳过
  if maybe_skip "${DNSMOS_MERGED_TSV}" "DNSMOS+MERGE"; then
    :
  else
    log "[RUN] DNSMOS step2 (docker if needed) + merge -> ${DNSMOS_MERGED_TSV}"
    ...
    # ====== 新增：如果 shard 文件已经齐全，则跳过 docker，只做 merge ======
    shopt -s nullglob
    SHARDS=( "${DNSMOS_SAVE_HOME}/dns_from_vad_"*"_${NUM_GPUS}.tsv" )
    shopt -u nullglob

    if [[ ${#SHARDS[@]} -eq ${NUM_GPUS} ]]; then
      log "[SKIP] DNSMOS docker (found ${#SHARDS[@]}/${NUM_GPUS} shards): ${DNSMOS_SAVE_HOME}"
    else
      log "[RUN] DNSMOS docker (need ${NUM_GPUS} shards, found ${#SHARDS[@]}). Will run scoring..."
    ...
    # 不管有没有跑 docker，都做一次 merge（只要 shard 在就能 merge）
    log "[RUN] Merge DNSMOS shards -> ${DNSMOS_MERGED_TSV}"
    pushd "${DNSMOS_DIR}" >/dev/null
    python3 merge_dns_by_json_order.py \
      --vad-json "${VAD_OUT_JSON}" \
