...
# YJ 音频清洗流程 v2（单镜像版）
#
# 相比 run_all4user.bash 的变化：
#   - 只使用一个 Docker 镜像 yj-pipeline-unified:latest
#   - 删除 docker_run_dnsmos / DNSMOS_DOCKER_IMAGE
#   - DNSMOS 打分直接在统一镜像内用绝对路径运行
#   - 其余逻辑完全不变
#
# 构建镜像：
#   cd docker && docker build -f Dockerfile.unified -t yj-pipeline-unified:latest .
############################################
...
# 统一镜像（替代原来的 RUNTIME_IMAGE + DNSMOS_DOCKER_IMAGE）
UNIFIED_IMAGE="yj-pipeline-unified:latest"
...
# 唯一的 docker runner：所有步骤统一用 UNIFIED_IMAGE
docker_run() {
  local gpu_spec="$1"
  local cmd="$2"
...
if [[ "$DO_DNSMOS" -eq 1 ]]; then
  if ! maybe_skip "${DNSMOS_MERGED_TSV}" "DNSMOS+MERGE"; then
    ...
    if [[ ${#SHARDS[@]} -ne ${local_num_gpus} ]]; then
      ...
      # 使用绝对路径调用脚本（不依赖 -w 工作目录；模型文件通过 __file__ 定位）
      DNSMOS_CMD="
        set -euo pipefail
