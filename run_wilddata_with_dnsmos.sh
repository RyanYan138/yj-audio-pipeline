#!/usr/bin/env bash
# 用法：bash run_wilddata_with_dnsmos.sh [GPU]
GPU=${1:-2}
...
"${CONDA_PREFIX}/bin/python" "${PROJECT_ROOT}/pipeline/streaming_pipeline.py" \
    ...
    --out_json          "${PROJECT_ROOT}/output/wilddata_with_dnsmos/labels.json" \
    --audio_dir         "${PROJECT_ROOT}/output/wilddata_with_dnsmos/audio" \
