#!/usr/bin/env python3
"""Optimized tar audio cleaning pipeline for non-vLLM/V100 inference.

Pipeline:
    tar decode -> FireRedVAD -> batched tiny Whisper LID
               -> duration buckets -> batched Fun-ASR-Nano -> JSON

The original pipeline remains untouched. This variant removes temporary WAV
round trips, runs LID once per source file, batches LID, batches every expensive
Fun-ASR-Nano GPU stage, and records stage-level throughput metrics.
"""
...
def producer(tar_paths: Sequence[str], vad_q: mp.Queue, stats_q: mp.Queue) -> None:
...
def lid_worker_batched(
    ...
    def flush_batch(batch: List[Tuple[dict, np.ndarray]]) -> None:
        nonlocal accepted_wavs, accepted_segments, lid_batches
        ...
        batch_array = feature_extractor(pending_audio)
        ...
        try:
            raw_results = model.detect_language(
                ctranslate2.StorageView.from_array(batch_array)
            )
        except Exception as exc:
            LOGGER.warning("[LID] batch failed: %s", exc)
            raw_results = [[("<|unknown|>", 0.0)]] * len(pending_items)
        infer_seconds += time.perf_counter() - detect_started
        lid_batches += 1
        ...
        for item, candidates in zip(pending_items, raw_results):
            ...
            if target_langs and language not in target_langs:
                continue
            if probability < min_lang_prob:
                continue
            accepted_wavs += 1
            base = {
                key: value
                for key, value in item.items()
                if key not in {"audio", "segments"}
            }
    ...
    stats_q.put(
        {
            ...
            "lid_accepted_wavs": accepted_wavs,
    ...
    LOGGER.info(
        "[LID] done: %d/%d wavs, %d segments, %d batches, inference %.3fs",
        accepted_wavs,
        source_wavs,
        accepted_segments,
        lid_batches,
...
def _funasr_bucket_phase2_offline(
    ...
    def process_batch(batch: List[dict]) -> List[dict]:
        ...
        language = None
        if force_language_from_lid:
            languages = {segment.get("text_lang") for segment in valid}
            if len(languages) == 1:
            ...
            records.append(
                {
                    ...
                    "text_lang": segment["text_lang"],
                    "transcribe": {"funasr-nano": result.get("text", "").strip()},
...
def funasr_bucket_worker(
    ...
    def load_existing(path: str) -> None:
        ...
        for record in candidates:
            segment_uuid = record.get("wav_uuid")
            if segment_uuid and segment_uuid not in done_uuids:
                done_uuids.add(segment_uuid)
                existing_results.append(record)

    if resume:
        load_existing(out_json)
        load_existing(checkpoint_path)
    ...
    buckets = StreamingDurationBuckets(
            ...
            records.append(
                {
                    ...
                    "num_sample": int(duration * SAMPLE_RATE),
                    "text_lang": segment["text_lang"],
                    "lang_prob": segment.get("lang_prob"),
                    "transcribe": {"funasr-nano": result.get("text", "").strip()},
                    "timestamp": {"funasr-nano": _timestamp_records(result)},
                }
            )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--lid_model_dir", required=True)
    ...
    parser.add_argument("--target_langs", nargs="*", default=["zh"])
    parser.add_argument("--min_lang_prob", type=float, default=0.90)
    ...
    parser.add_argument("--force_language_from_lid", action="store_true")
...
def _main_offline() -> None:
    ...
    phase2_stats = _funasr_bucket_phase2_offline(
        ...
        force_language_from_lid=args.force_language_from_lid,
    )
...
def main() -> None:
    args = parse_args()
    out_dir = str(Path(args.out_json).parent)
    checkpoint_path = os.path.join(out_dir, "phase2_ckpt.jsonl")
    metrics_path = args.metrics_json or os.path.join(out_dir, "metrics.json")
    ...
    pipeline_started = time.perf_counter()
    LOGGER.info(
        "=== Streaming pipeline: VAD -> dynamic batch LID -> duration buckets -> Nano ==="
    )
    try:
        for process in processes:
            process.start()
    ...
    with open(metrics_path, "w", encoding="utf-8") as handle:
        json.dump(metrics, handle, ensure_ascii=False, indent=2)
    LOGGER.info(
        "Complete: %.2fs total, cold end-to-end %.2fX, ASR compute %.2fX, output=%s",
        total_seconds,
        metrics["end_to_end_x"] or 0.0,
