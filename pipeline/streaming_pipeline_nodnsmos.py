...
def lid_worker(lid_q: mp.Queue, asr_q: mp.Queue,
    ...
    try:
        ...
        logger.info(f"[LID] fallback cpu/int8: {_e}")
    ...
    while True:
        ...
        try:
            if audio_id not in lang_cache:
                ...
                # 检查样本数防止 faster-whisper 内存爆炸
                if audio.ndim != 1:
                    audio = audio.mean(axis=-1) if audio.ndim == 2 else audio.flatten()
                if len(audio) < 1600 or len(audio) > 15 * SAMPLE_RATE:
                    logger.warning(f"[LID] {audio_id} bad length: {len(audio)} samples, ndim={audio.ndim}")
                    continue
                if len(audio) > 12 * SAMPLE_RATE:
                    audio = audio[:12 * SAMPLE_RATE]
                import signal as _sig
                def _timeout(s, f): raise TimeoutError("lid timeout")
                _sig.signal(_sig.SIGALRM, _timeout)
                _sig.alarm(30)
                try:
                    _, info = model.transcribe(audio, task="transcribe", beam_size=1)
                finally:
                    _sig.alarm(0)
                lang = info.language
...
def main():
    ...
    logger.info(f"Found {len(audio_files)} audio files (shard {args.start_idx}:{args.end_idx})")
