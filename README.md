...
你给用户使用的脚本本质上就是：
把两个镜像名、GPU、Whisper 模型路径和 LID 模型路径都集中放在顶部配置区。
现有脚本里这些关键配置本来就集中在前面，比如镜像名、GPU、Whisper 模型目录和 LID 模型目录。
...
当前普通用户方案是 **双镜像运行**，不要求用户自己手动配置 Conda 环境。
镜像导入后，直接通过脚本调用即可。你给的使用文档也是基于“先导入镜像，再改配置，再运行”的流程。
